...
function sample<T extends Transaction>(hub: Hub, transaction: T, samplingContext: SamplingContext): T {
  ...
  if (transaction.sampled !== undefined) {
    transaction.tags = { ...transaction.tags, samplingMethod: 'explicitly_set' };
    return transaction;
  }
  ...
  if (typeof options.tracesSampler === 'function') {
    ...
    transaction.tags = {
      ...transaction.tags,
      samplingMethod: 'client_sampler',
      sampleRate: String(Number(sampleRate)),
    };
  } else if (samplingContext.parentSampled !== undefined) {
    sampleRate = samplingContext.parentSampled;
    transaction.tags = { ...transaction.tags, samplingMethod: 'inheritance' };
  } else {
    sampleRate = options.tracesSampleRate;
    // cast the rate to a number first in case it's a boolean
    transaction.tags = { ...transaction.tags, samplingMethod: 'client_rate', sampleRate: String(Number(sampleRate)) };
  }
